...
<div class="card p-3 mb-5 pb-3 mt-4">
    <div class="card-body text-center">
        ...
        <div class="form-group mb-3 px-4">
            <form class="d-inline" method="post" action="{{ route('couriers.destroy', $courier->id)}}">
                @method('delete')
                @csrf
                <button type="submit" class="btn btn-outline-danger mx-2" onclick="return confirm('Hapus Courier ?')">Delete</button>
            </form>
            <a href="{{ url('admin/couriers')}}" class="btn btn-outline-secondary mx-1">All Courier List</a>
        </div>
        <div class="form-group mb-3">
            <form class="input-group mb-3" method="post" action="{{ route('couriers.update', $courier->id) }}">
                @method('patch')
                @csrf
